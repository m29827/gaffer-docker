...
function init_project() {
    ...
    # Work around relative path from Gaffer Chart to HDFS Chart - otherwise this won't resolve 
    cd ${OPERATOR_BUILD_DIR} && mkdir helm-charts && tar xvf ${ROOT_DIR}/kubernetes/gaffer/charts/hdfs-${APP_VERSION}.tgz -C helm-charts

    operator-sdk init --plugins=helm.sdk.operatorframework.io/v1 \
        --project-name ${OPERATOR_NAME} \
        --domain=gaffer.gchq.gov.uk \
        --version=v1 \
        --helm-chart=${ROOT_DIR}/kubernetes/gaffer
}
